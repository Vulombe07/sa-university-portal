...
const filters = ["All (12)", "Eligible (9)", "Not Eligible (2)", "Partial (1)"];
...
export default function Eligibility({ setPage, eligibilityData = eligibilityFixture }) {
  ...
  const visibleProgrammes = eligibilityData.recommendations.filter((programme) => `${programme.name} ${programme.university}`.toLowerCase().includes(programmeQuery.toLowerCase()));
    ...
    <AppLayout setPage={setPage} activePage="eligibility">
      <div className="concept-page eligibility-concept-page">
        ...
        <section className="concept-metric-grid">
          <article className="concept-metric-card"><span>Your current APS</span><strong>{eligibilityData.apsScore}</strong><small>Calculated from your marks</small><button type="button" className="concept-link" onClick={() => setPage("profile")}>View/edit marks <ChevronRight size={13} /></button></article>
          <article className="concept-metric-card"><span>Eligible programmes</span><strong>12</strong><small>Based on your current marks</small><button type="button" className="concept-link" onClick={() => setActiveFilter("Eligible (9)")}>View eligible <ChevronRight size={13} /></button></article>
        ...
        <section className="concept-table-panel card-panel">
          ...
          <div className="concept-tabs">{filters.map((filter) => <button type="button" key={filter} className={activeFilter === filter ? "active" : ""} onClick={() => setActiveFilter(filter)}>{filter}</button>)}</div>
          ...
          <div className="table-footer">Showing {visibleProgrammes.length} programmes <button type="button" className="concept-link" onClick={() => setPage("programmes")}>View 12 programmes <ChevronRight size={13} /></button></div>
        </section>
      </div>
    </AppLayout>
  );
